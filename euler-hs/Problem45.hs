...
trim :: (Num a, Ord a) => (a1 -> a) -> a -> [a1] -> [a]
triangular, pentagonal, hexagonal :: Integral a => a -> a
isTriangular, isPentagonal, isHexagonal :: Integral a => a -> Bool
main :: IO ()

trim f n = takeWhile (> n + 1) .
    dropWhile (< n - 1) .
    map f
